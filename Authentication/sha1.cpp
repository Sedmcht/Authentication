...
string sha1(string message) {
    ...
    uint totalBlockCount = message.size() / one_block_size_bytes;

    // подсчет, сколько байт нужно, чтобы дополнить последний блок
    uint needAdditionalBytes =
        one_block_size_bytes - (message.size() - totalBlockCount * one_block_size_bytes);
